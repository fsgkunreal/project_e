// Copyright (c) 2024 Betide Studio. All Rights Reserved.

#pragma once

#include "CoreMinimal.h"
#include "SIK_BaseWebApi.h"
#include "SIK_UpdateIncompatibleStatus.generated.h"

/**
 * 
 */
UCLASS()
class STEAMWEB_API USIK_UpdateIncompatibleStatus : public USIK_BaseWebApi
{
    GENERATED_BODY()

public:
	
	//Updates the incompatible status on the published file. "Incompatible" items are hidden from community hubs and user profiles, but can still be accessed by a direct link.
	UFUNCTION(BlueprintCallable, Category = "Steam Integration Kit|Web|PublishedFile", meta = (BlueprintInternalUseOnly = "true"))
	static USIK_UpdateIncompatibleStatus* UpdateIncompatibleStatus(const FString& Key, const int64& PublishedFileId, const int32& AppId, const bool& Incompatible);

private:
    virtual void Activate() override;
	FString Var_Key;
	int64 Var_PublishedFileId;
	int32 Var_AppId;
	bool Var_Incompatible;
};
