...
#include "SIK_RankedByVote.h"

USIK_RankedByVote* USIK_RankedByVote::RankedByVote(const FString& Key, const int64& SteamId, const int32& AppId,
    const int32& StartIdx, const int32& Count, const int32& TagCount, const int32& UserTagCount,
    const bool& HasAppAdminAccess, const int32& FileType, const FString& Tag0, const FString& UserTag0)
{
    USIK_RankedByVote* Node = NewObject<USIK_RankedByVote>();
    Node->Var_Key = Key;
...
void USIK_RankedByVote::Activate()
{
    Super::Activate();
    FString URL = FString::Printf(TEXT("%s/ISteamPublishedItemSearch/RankedByVote/v1/"), *APIEndpoint);
    ...
    Request->OnProcessRequestComplete().BindUObject(this, &USIK_RankedByVote::OnResponseReceived);
