...
#include <SteamSdk/Includes/steam/steamuniverse.h>
...
void FOnlineAsyncTaskSteamCreateServer::Tick() 
{
	...
	if (!bInit)
	{
		...
		if (Session != nullptr && SteamGameServerPtr != nullptr)
		{
			...
			FString TempServerModDir;
			if (!GConfig->GetString(TEXT("OnlineSubsystemSteam"), TEXT("ServerGameDir"), TempServerModDir, GEngineIni))
			{
				TempServerModDir = UE_PROJECT_STEAMGAMEDIR;
			}
			FString TempServerName;
			if (!GConfig->GetString(TEXT("OnlineSubsystemSteam"), TEXT("ServerName"), TempServerName, GEngineIni))
			{
				TempServerName = UE_PROJECT_STEAMPRODUCTNAME;
			}
			FString TempServerDesc;
			if (!GConfig->GetString(TEXT("OnlineSubsystemSteam"), TEXT("ServerDescription"), TempServerDesc, GEngineIni))
			{
				TempServerDesc = UE_PROJECT_STEAMGAMEDESC;
			}
			SteamGameServerPtr->SetModDir(TCHAR_TO_UTF8(*TempServerModDir));
			SteamGameServerPtr->SetProduct(TCHAR_TO_UTF8(*TempServerName));
			SteamGameServerPtr->SetGameDescription(TCHAR_TO_UTF8(*TempServerDesc));
			...
			{
				SteamGameServerPtr->SetModDir(TCHAR_TO_UTF8(*UE_PROJECT_STEAMGAMEDIR));
			}
			FString TempServerName;
			if (GConfig->GetString(TEXT("OnlineSubsystemSteam"), TEXT("ServerName"), TempServerName, GEngineIni))
			{
				SteamGameServerPtr->SetServerName(TCHAR_TO_UTF8(*TempServerName));
			}
			else
			{
				SteamGameServerPtr->SetServerName(TCHAR_TO_UTF8(*UE_PROJECT_STEAMPRODUCTNAME));
			}
			FString TempServerDesc;
			if (GConfig->GetString(TEXT("OnlineSubsystemSteam"), TEXT("ServerDescription"), TempServerDesc, GEngineIni))
			{
			...
			{
				SteamGameServerPtr->SetGameDescription(TCHAR_TO_UTF8(*UE_PROJECT_STEAMGAMEDESC));
			}*/
...
void FOnlineAsyncTaskSteamFindServerBase::CreateQuery(MatchMakingKeyValuePair_t** OutFilter, int32& NumFilters)
{
	...
	*OutFilter = new MatchMakingKeyValuePair_t[MaxFilters];
	...
	if (GConfig->GetString(TEXT("OnlineSubsystemSteam"), TEXT("ServerGameDir"), TempServerModDir, GEngineIni))
	{
		FCStringAnsi::Strncpy(Filters[NumFilters].m_szValue, TCHAR_TO_ANSI(*TempServerModDir), ValueSize);
	}
	else
	{
		FCStringAnsi::Strncpy(Filters[NumFilters].m_szValue, UE_PROJECT_STEAMGAMEDIR, ValueSize);
	}
	UE_LOG(LogOnline, Verbose, TEXT("Searching for servers in game dir %s"), UTF8_TO_TCHAR(Filters[NumFilters].m_szValue));
	NumFilters++;
...
//#endif
		...
		{
			UE_LOG(LogOnline, Warning, TEXT("FOnlineAsyncTaskSteamFindServerBase::Tick - RequestServerList failed"));
			// Invalid API call
			bIsComplete = true;
			bWasSuccessful = false;
		}
