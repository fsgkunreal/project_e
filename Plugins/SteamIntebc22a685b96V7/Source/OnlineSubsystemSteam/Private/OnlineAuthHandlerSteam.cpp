...
#if ENGINE_MAJOR_VERSION >= 5 && ENGINE_MINOR_VERSION > 2
	ResendingPacket.Type = (Handler->Mode == UE::Handler::Mode::Server) ? ESteamAuthMsgType::ResendKey : ESteamAuthMsgType::ResendResult;
#else
	ResendingPacket.Type = (Handler->Mode == Handler::Mode::Server) ? ESteamAuthMsgType::ResendKey : ESteamAuthMsgType::ResendResult;
#endif
...
#if ENGINE_MAJOR_VERSION >= 5 && ENGINE_MINOR_VERSION > 2
	else if (Handler && Handler->Mode == UE::Handler::Mode::Server && Header.Type == ESteamAuthMsgType::ResendKey)
#else
