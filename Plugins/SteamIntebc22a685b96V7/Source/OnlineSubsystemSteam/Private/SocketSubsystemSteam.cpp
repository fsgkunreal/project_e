...
void FSocketSubsystemSteam::Shutdown()
{
	for (int32 ConnIdx=SteamConnections.Num()-1; ConnIdx>=0; ConnIdx--)
	{
		...
		{
			USIKSteamNetConnection* SteamConn = CastChecked<USIKSteamNetConnection>(SteamConnections[ConnIdx].Get());
 ...
 */
void FSocketSubsystemSteam::RegisterConnection(USIKSteamNetConnection* Connection)
{
 ...
 */
void FSocketSubsystemSteam::UnregisterConnection(USIKSteamNetConnection* Connection)
{
	check(!Connection->bIsPassthrough);
