...
USIKSteamNetDriver::USIKSteamNetDriver(const FObjectInitializer& ObjectInitializer) :
	Super(ObjectInitializer),
	bIsPassthrough(false)
{
}

void USIKSteamNetDriver::PostInitProperties()
{
	Super::PostInitProperties();
}

bool USIKSteamNetDriver::IsAvailable() const
{
...
ISocketSubsystem* USIKSteamNetDriver::GetSocketSubsystem()
{
	return ISocketSubsystem::Get(bIsPassthrough ? PLATFORM_SOCKETSUBSYSTEM : STEAM_SUBSYSTEM);
}

bool USIKSteamNetDriver::InitBase(bool bInitAsClient, FNetworkNotify* InNotify, const FURL& URL, bool bReuseAddressAndPort, FString& Error)
{
	if (bIsPassthrough)
	{
...
bool USIKSteamNetDriver::InitConnect(FNetworkNotify* InNotify, const FURL& ConnectURL, FString& Error)
{
	ISocketSubsystem* SteamSockets = ISocketSubsystem::Get(STEAM_SUBSYSTEM);
	if (SteamSockets)
	{
...
bool USIKSteamNetDriver::InitListen(FNetworkNotify* InNotify, FURL& ListenURL, bool bReuseAddressAndPort, FString& Error)
{
	ISocketSubsystem* SteamSockets = ISocketSubsystem::Get(STEAM_SUBSYSTEM);
...
void USIKSteamNetDriver::Shutdown()
{
	if (!bIsPassthrough)
	{
...
bool USIKSteamNetDriver::IsNetResourceValid()
{
